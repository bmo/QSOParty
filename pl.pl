...
my @W7DX_BONUS_MODES=('CW','PH','DG');   # for what modes do we count W7DX bonus?
	...
	while ($idx < $#LOG) {
		...
		if ($creator eq 'GENLOG') {
		 ...
		 if ($is_wa < 0) {
			...
			if (my $dxiam = get_dxcc_entity($iam)) {
				...
				if ($dxiam ne 'K') {
				    print "Not in the US, using NON-WA\n\n";
				    $is_wa = 0;
				    last SCORETYPE;
				} 
			}
	 ...
	 # printf "MODE %s -- %s\n",$mode,$MODE_SCORE{$mode_category};
		 ...
		 print "Dupe - $wrkd $band $mode $rexch\n";
			...
			# printf "[%s]\n",$wrkd;
			# record that we worked W7DX
            if ($wrkd eq 'W7DX') {
			   if ($debug_scoring) { print "\t\t\t\tWorked W7DX - mode: $mode\n"; }
			   $BONUS{$mode}++;
			}
