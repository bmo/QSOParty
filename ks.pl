...
my $COUNT_QSOPARTY_STATE_AS_MULT = 1;            # if non-zero, when we get an in-state county for an in-state station, count it as a state multiplier

# what counts as a mult for an in-state station?
my $IN_STATE_STATION_MULTIPLIERS = { us_states_prov => 1, dx => 1, own_state => 1, all_dx_as_one => 1};
# what counts as a mult for an out-of-state station?
my $OUT_OF_STATE_STATION_MULTIPLIERS = { counties => 1};

# these are used to score
my $MULTIPLIERS = { 0 => $OUT_OF_STATE_STATION_MULTIPLIERS,
		    1 => $IN_STATE_STATION_MULTIPLIERS } ;

#print "MULTIPLIER ",Dumper(%MULTIPLIERS);
#print Data::Dumper->Dump([$MULTIPLIERS], ['*multipliers']);
#print Data::Dumper->Dump([%IN_STATE_STATION_MULTIPLIERS], ['*in_state']);
#print Dumper($IN_STATE_STATION_MULTIPLIERS);
#print Dumper($MULTIPLIERS->{'0'});
...
$FIELD_OFFSETS{'GUESS'}="53:10,70:4,48:6";
...
sub score_log {
	...
	while ($idx < $#LOG) {
      ...
      if ($qline =~ /^([\w,-]+)\:\s*(.*)\s*/) {
		...
		$claimed_score = substr($qline,15);
		 ...
		 GETFIELDS: {
			 ...
			 if (substr($allfields[5],$iam) < 0) {
			     printf "Couldn't guess the QSO line format for $filename - $iam is not the same as $allfields[5]\n";
	 ...
	 #printf "MODE %s -- %s\n",$mode,$MODE_SCORE{$mode_category};
	     ...
	     if ($debug_scoring) { print "$wrkd\t$band\t$mode\t$sexch\t$rexch\n"; }
	     ...
	     if ($MODE_SCORE{$mode_category}!~/$mode/) {
	      ...
	      if (defined($sexch)) { $dupeindex .= $sexch; }
	      $ISADUPE{$dupeindex}++;
	    ...
	    if (grep {$_->{call} eq $wrkd} @BONUS_STATIONS) {
			...
			if (defined($COUNTY{$rexch})) {
			    if ($MULTIPLIERS->{$is_in_state}{counties}) {
				if ($debug_scoring) {
				...
				print "\t\t\t\tNOT counting COUNTY $rexch  \n";
			    }
			    if ($MULTIPLIERS->{$is_in_state}{own_state}) {
				# Count state as a mult too if needed
				if ($COUNT_QSOPARTY_STATE_AS_MULT ) {
				    $statemult{$STATEQSOPARTY}++;
				    if ($debug_scoring) {
						print "\t\t\t\tCounting $STATEQSOPARTY state  \n";
				    }
				}
			    }
   	        ...
   	        # right now, just count anything...
				...
				if (($rexch eq $STATEQSOPARTY) || (!defined($DXCC_ENTITY{$rexch}) && !(defined($STATEPROV{$rexch})))) {
					print "Suspect exchange $rexch for QSO: [$rexch] \n",$qline,"\n";
					    ...
					    if ($MULTIPLIERS->{$is_in_state}{dx}) {
						$dxmult{get_dxcc_entity($wrkd)}++;
					    } else {
						 print "NOT COUNTING - $wrkd - QSO:\n",$qline,"\n"; 
					    }
					}
				} else {
				  if ($is_in_state) {
					...
					if (($om eq "VE") || ($om eq 'K') || ($om eq 'KH6') || ($om eq 'KL')) {
						if (defined($STATEPROV{$rexch})) {
						    if ($MULTIPLIERS->{$is_in_state}{us_states_prov}) {
							if ($debug_scoring) {
							...
							{ print "NOT COUNTING state/province- $rexch - QSO:\n",$qline,"\n"; }
...
BAILOUT:
	  ...
	  if ($MULTIPLIERS->{$is_in_state}{all_dx_as_one}) {
	      $dx_cnt = ($dx_cnt > 0);
	      printf "(Counting all DX as 1)\n";
	  }
	  printf "DX:\t\t%10d\n",$dx_cnt;
